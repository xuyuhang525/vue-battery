<template>
  ...
  <div class="content">
    ...
    <div id="depth" class="content"></div>
    <yh-dialog
      ref="details"
      :showForm.sync="show"
      :title="title + '查询'"
...
export default {
    ...
    return {
      ...
      options: [
        ...
        },
        {
          ...
          label: "容量保持率",
          unit: "%",
          src: "capacityRetentionRate",
        },
        {
          value: "4",
          label: "能量保持效率",
          unit: "%",
          src: "energyRetentionEfficiency",
        },
        {
          value: "5",
          label: "温升-正极",
  ...
  mounted() {
    getBatteryTitle({ type: "2" }).then((res) => {
      let arr = [];
      res.data.map((item) => {
        arr.push({ value: item, label: item * 100 });
      });
      this.optionsTitle = arr;
      if (res.data.length != 0) {
        this.queryParams.title = res.data[0];
        this.titleValue = res.data[0] * 100;
      }
  ...
  methods: {
    ...
    init(data) {
      ...
      this.myChart = echarts.init(document.getElementById("depth"));
      let option = {
        title: {
          text: "放电深度信息",
          padding: [5, 0, 0, 40],
        ...
        tooltip: {
          ...
          formatter: function (params) {
            return `<div class=chartLabel>
              <div class=label>DOD：${that.titleValue}%</div>
